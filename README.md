...
5. 每份 CSV 都包含当前病例的**全部已记录数据**。选择文件夹后，应用每次更新 `病例_HR_会话ID_最新完整记录.csv`，保留最新完整版本，避免长期重复保存全量副本。Chrome 下载模式仍每次生成带 UTC 导出时间的新文件，合并时按“记录ID”去重。需要历史版本时可额外手动导出；用药及完整修订审计仍通过相应 CSV / JSON 按钮导出。
