...
PetOR Monitor 在浏览器本地识别监护仪上的**心率（HR）和呼吸率（RR）**，支持摄像头与本地视频。两项参数分别框选 ROI，按同一画面/视频时间槽生成记录，支持趋势、简体中文播报、人工修订审计、CSV/JSON 导出、每 5 分钟自动保存和本地草稿恢复。HR 参考 CSV 对比保持独立。

SpO₂、PR、NIBP、EtCO₂、FiCO₂ 和 TEMP 六项尚未启用。RR 本次加入，尚未完成真实设备视频的准确率验收；预设中的 RR 区域只用于初始定位，使用前必须手动校准并核对。

### 呼吸率采集

1. 打开摄像头或导入视频，在“校准识别区域”点击 **RR 呼吸率 重画**，仅框住呼吸率数字，避免相邻参数。HR 也需独立校准。
2. 实时 RR 要求两次连续一致；离线每项每槽要求严格多数（标准五帧、逐秒三帧），裁掉数字导致的 `18/8` 等冲突保留待核对。
3. RR 只接受 0–180 次/分钟的唯一整数，0 和个位数可以识别；小数、负数、多个数字及字母替代数字会拒识。这是 OCR 工程范围，不是临床正常范围。
4. 点击“开始自动记录”；选择本地保存文件夹后，HR 与 RR 每 5 分钟保存到同一份完整 CSV。未识别保持为空，旧心率记录也不会补造 RR。
5. 点击记录表的 RR 单元格可人工修订，必须填写原因。JSON 审计导出使用 `petor-monitor/vital-record-export/2.0`，包含两项参数及修订轨迹。

摄像头的一轮识别使用一张冻结画面，两项结果全部完成后一起发布，防止定时记录混入不同帧；一项 OCR 失败不会跳过另一项。仍使用一个 OCR worker 串行处理，单轮裁剪识别次数由 1 次增加为 2 次，实际 CPU 和耗时需在目标设备测试。详见 [RR 验证说明](docs/RESPIRATORY_RATE_VALIDATION.md)。
...
1. 在桌面 Chrome 打开本地应用，进入“心率与呼吸率记录”区域。“每 5 分钟自动保存 CSV”默认开启。
...
5. 每份 CSV 都包含当前病例的**全部已记录数据**。选择文件夹后，应用每次更新 `病例_HR_RR_会话ID_最新完整记录.csv`，保留最新完整版本，避免长期重复保存全量副本。Chrome 下载模式仍每次生成带 UTC 导出时间的新文件，合并时按“记录ID”去重。需要历史版本时可额外手动导出；用药及完整修订审计仍通过相应 CSV / JSON 按钮导出。
...
| HR采集时间ISO_UTC / RR采集时间ISO_UTC | 摄像头抓取画面的时刻；离线多帧共识使用其所属时间槽时刻 |
| 视频时间 | 视频相对起点的秒数，不是分析所用的时间 |
| 写入分析时间ISO_UTC | 程序实际写入 / 完成分析的时刻，与采集时间分开 |
| 视频起始时间ISO_UTC | 用户输入的视频第 0 秒时间 |

导入视频时，请填写“视频第 0 秒时间”，应用按“起点 + 视频相对秒”建立绝对时间。如果没有填写，CSV 的绝对对齐时间、对齐时间戳和 HR / RR 绝对采集时间保持空白，只导出视频相对秒，避免把分析当天误当作拍摄日期。原有可读时间列按电脑本地时区显示；跨设备对齐请使用 UTC / 毫秒列，并确认设备时钟一致。
...
当前 HR + RR 双指标：24 小时 289 行合成记录约占 452 KB 草稿、74 KB CSV；7 天约 3.15 MB 草稿。详见 [双指标容量与验证范围](docs/RESPIRATORY_RATE_VALIDATION.md)。下方 HR 单项基准保留用于历史对照。

先按每天 / 每个病例归档使用，不将当前 Web 版当作无人值守的 24×7 采集服务。保持接电、Chrome 页面可见、电脑不休眠，开始记录后确认页面显示“屏幕常亮中”。系统释放常亮、锁屏、合盖或把页面切到后台时可能中断；恢复后必须确认画面并手动继续记录。

长期运行优先选择本地保存文件夹，确认有实际 CSV 文件更新；刷新页面后重新选择目录。每天或病例结束时保存完整 CSV 和审计 JSON 后再新建病例。记录表默认跟随最新页，每页最多 50 行，翻页可以查看 / 修订历史；CSV 导出与打印仍包含全部记录。浏览器草稿较大时会提前提示归档。

2026-09-15 完成 24 小时、7 天、30 天的**模拟记录量测试**和模拟 24 小时的 288 次自动保存调度验证，尚未完成真实摄像头的 24 小时耐久测试。默认五分钟采样，24 小时含起始点共 289 行；模拟草稿按保守 UTF-16 口径约 0.38 MiB，完整 CSV 约 62 KiB。30 天同一会话仍触发测试环境的浏览器草稿配额失败，因此不承诺同一病例无限增长。详情见 [长时间运行检查报告](docs/LONG_RUN_VALIDATION.md)。
...
1. 填写病例信息（可选）。
2. 点击“演示模式”，检查 HR / RR 模拟读数和趋势。
3. 点击“测试播报”，听到简体中文后确认页面显示已开启记录后播报。
4. 留档间隔选“10 秒（验收演练）”，再点“开始自动记录”。
5. 确认首行立即生成，后续每 10 秒新增一行并语音复诵。
6. 点击任意表格读数，输入新值与修订原因，确认“已修订”。
7. 点击“导出 CSV”和“JSON”，核对 HR、RR、计划时间、实际时间、置信度、状态与审计数据。

真实监护仪在线流程：点击“摄像头” → 允许权限 → 分别校准 HR 与 RR ROI → 等待HR 与 RR 各自连续两次识别一致 → 开始记录。摄像头仅在 HTTPS 或 localhost 可用。

真实视频离线流程：点击“导入视频” → 选择本机 MP4/MOV → 选择对应的监护仪布局并分别校准 HR 与 RR ROI → 选择 5 分钟、1 分钟、30 秒或 1 秒间隔 → 可选填写“视频第 0 秒时间” → 开始离线分析 → 逐格人工核对 → 导出 CSV/JSON。视频使用媒体时间抽帧，不会启动墙上时间调度器；5 分钟、1 分钟和 30 秒模式使用每项每槽五帧共识，1 秒模式使用每项每槽三帧共识。每次抽帧必须由 `requestVideoFrameCallback` 返回的 `metadata.mediaTime` 与目标媒体时间匹配后才进入 OCR；可能截取旧帧的 `requestAnimationFrame`/定时回退已移除，不支持或超时会明确失败且不生成伪记录。只有满足唯一整数、范围、置信度、多帧一致性和跨槽跳变确认门槛的值才会自动接受；无法可靠识别时保留空值或“待核对”，绝不钳制、猜值或沿用上一槽的旧值。重跑同名视频时只有完整成功后才会事务式替换旧结果，失败或取消会保留原记录。
...
1 秒模式是耗时的本地离线任务，不是实时报警或“一秒内出结果”。例如 602.333 秒视频会建立 `0…602` 共 603 个 HR 时间槽，最多执行 3,618 次 HR + RR OCR；实际耗时取决于视频解码、CPU、浏览器和散热，不承诺固定速度，尤其不保证手机端可以完成或达到桌面端性能。长视频优先使用接电的桌面端，运行期间保持页面在前台并防止设备锁屏；用户可随时停止，系统只保留已完成的整个时间槽，不写入未完成或插值的 HR。先用 5 分钟模式抽查 ROI 与拒识规则，再运行逐秒审计。
